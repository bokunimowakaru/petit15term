...
int strnicmp(const char* a, const char* b, int c){ return strncasecmp(a,b,c); }
    ...
    if (argc >= 5)
    {
        for (i = 1; i < argc - 3; i++)
        {
            ...
            if(strnicmp(argv[i],"-debug", 6) == 0)
            {
                char* num;
                num = argv[i] + 6;
                while(*num && isdigit((int)*num) == 0) num++;
                if(isdigit((int)*num) != 0) debug_level = atoi( num);
                else debug_level = 4;
                DebugPrintf(3, "Turn on debug, level: %d.\n", debug_level);
                continue;
            }
            // isdigit(*num) -> isdigit((int)*num)‚É•ÏX by ‘–ì˜j
